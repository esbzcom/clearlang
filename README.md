...
```
Deterministic ClearLang source formatting:
```
...
```
Note: production release is fail-closed; this command only succeeds when
`project.clg_version` is compatible with the running `clg` and the selected
entrypoint uses only production-allowed proved std surfaces.
...
```
Timeout/cancellation guidance and schema-version contract are pinned in `docs/ide/vscode-cli-profile.md`.
...
`clg release` reads non-secret defaults from `clg.project.json` (generated by `clg strict init`): `project.entry`, `release_defaults.advisory_as_of`, `release_defaults.key_id`, `release_defaults.out_dir`, and `release_defaults.trust_policy`.
`clg.project.json` schema v1 also stores project metadata (`name`, `description`, `version`, `clg_version`, `entry`, website/contact) and user-authored dependency requirements for `clg pkg lock`:
