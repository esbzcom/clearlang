...
fn ci_workflow_enforces_validation_and_tests() {
    ...
    for cmd in [
        ...
        "cargo test -p clg-cli --test prefixed_literal_parity",
    ...
    let parity_include = as_sequence(
        ...
        "milestone3-proof-parity matrix include",
    );
    let mut has_windows = false;
    let mut has_linux = false;
    let mut has_macos = false;
    for entry in parity_include {
        let item = as_mapping(entry, "milestone3-proof-parity include item");
        let os = mapping_get_str(item, "os", "milestone3-proof-parity include item");
        let platform = mapping_get_str(item, "platform", "milestone3-proof-parity include item");
    ...
    assert!(
        ...
        "milestone3 proof parity matrix must include windows + linux + macos parity targets"
    );
    ...
    assert!(
        ...
        .contains("tmp/proof-parity/${{ matrix.platform }}.json"),
        "milestone3 proof parity snapshot should emit deterministic matrix artifact path"
    );
    ...
    let (_, parity_compare_step) = find_step(
        ...
        "Compare milestone_3 proof parity artifacts",
    );
    let parity_compare_run = mapping_get_str(
        parity_compare_step,
        "run",
        "Compare milestone_3 proof parity artifacts step",
    );
    assert!(
        parity_compare_run.contains(
            "cmp --silent tmp/proof-parity/windows/windows.json tmp/proof-parity/linux/linux.json"
        ),
        "milestone3 proof parity compare job should enforce windows/linux artifact parity"
    );
    assert!(
        parity_compare_run.contains(
            "cmp --silent tmp/proof-parity/windows/windows.json tmp/proof-parity/macos/macos.json"
        ),
        "milestone3 proof parity compare job should enforce windows/macos artifact parity"
    );
    ...
    assert!(
        has_windows && has_linux && has_macos,
        "milestone3 test parity matrix must include windows + linux + macos parity targets"
    );
    ...
    assert!(
        ...
        "milestone3 test parity compare job should enforce windows/macos artifact parity"
    );
    ...
    assert!(
        ...
        "milestone3 release-train gate should require checks + proof parity + proof parity compare + test parity compare jobs"
    );
}
