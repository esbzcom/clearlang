...
#[test]
fn import_unknown_std_eth_item_reports_c021_with_package_guidance() {
    let tmp = tempdir().unwrap();
    let root = tmp.path();

    let main_src = r#"
        import std::eth::{missing}
        function main() -> Int { 0 }
    "#;
    let main_path = root.join("main.clear");
    fs::write(&main_path, main_src.trim()).expect("write main");
    ...
    let message = e0.get("message").and_then(|s| s.as_str()).unwrap_or("");
    assert!(
        message.contains("symbol `std::eth::missing` moved to bundled std package `std::eth`"),
        "expected bundled std eth migration guidance, got: {message}"
    );
}
...
#[test]
fn import_unknown_std_cosmos_item_reports_c021_with_package_guidance() {
    let tmp = tempdir().unwrap();
    let root = tmp.path();

    let main_src = r#"
        import std::cosmos::{missing}
        function main() -> Int { 0 }
    "#;
    let main_path = root.join("main.clear");
    fs::write(&main_path, main_src.trim()).expect("write main");
    ...
    assert!(
        message.contains(
            "symbol `std::cosmos::missing` moved to bundled std package `std::cosmos`"
        ),
        "expected bundled std cosmos migration guidance, got: {message}"
    );
}
...
#[test]
fn import_std_cosmos_chain_type_item_allows_build() {
    let tmp = tempdir().unwrap();
    let root = tmp.path();

    let main_src = r#"
        import std::cosmos::{Addr}
        function id(a: Addr) -> Addr { a }
        function main() -> Int { 0 }
    "#;
    let main_path = root.join("main.clear");
    fs::write(&main_path, main_src.trim()).expect("write main");
