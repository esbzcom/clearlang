...
fn milestone2_readiness_evidence_includes_security_runbook_signing_and_release_notes() {
    ...
    assert!(
        content.contains("## Concrete Signed Artifact Validation"),
        "readiness evidence should include concrete signed artifact validation section"
    );
    assert!(
        content.contains("milestone2_readiness_gate_exercises_signed_artifact_roundtrip"),
        "readiness evidence should reference the concrete milestone_2 signed artifact gate test"
    );
}
