...
#[test]
fn shared_std_cosmos_runtime_loader_tamper_missing_artifact_reports_r012() {
    let (tmp, wasm_path) = setup_shared_std_runtime_loader_fixture("std::cosmos", "1.2.0", 0, 0);
    let root = tmp.path();
    fs::remove_file(root.join("std-packages").join("std-cosmos-1.2.0.wasm"))
        .expect("remove shared std cosmos artifact");
    let stdout = run_json_error_output(&wasm_path);
    assert_first_error_code(stdout.as_slice(), "R012");
}
