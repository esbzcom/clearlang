...
use anyhow::Result;
use clg_ast::{BinOp, Block, Expr, Func, MatchArm, MatchPat, Span, Stmt, Type};
use clg_ir::{
...
fn lower_block_expr<'a>(ctx: &mut LowerCtx<'a>, block: &'a Block) -> Result<Value> {
    let mut inserted: Vec<(&'a str, Option<Value>)> = Vec::new();
...
fn lower_block_stmt<'a>(ctx: &mut LowerCtx<'a>, block: &'a Block) -> Result<()> {
    ...
    let result = (|| -> Result<()> {
        ...
        if let Some(tail) = &block.tail {
            let _ = lower_expr(ctx, tail.as_ref())?;
        }
        Ok(())
    })();
    restore_scope(ctx, inserted);
...
fn lower_block_statements<'a>(
    ctx: &mut LowerCtx<'a>,
    block: &'a Block,
    inserted: &mut Vec<(&'a str, Option<Value>)>,
) -> Result<()> {
...
fn restore_scope<'a>(ctx: &mut LowerCtx<'a>, inserted: Vec<(&'a str, Option<Value>)>) {
...
fn lower_while_stmt<'a>(
    ...
    let inv_head = lower_expr(ctx, invariant)?;
    ...
    if let Some(var_expr) = variant {
        let v_before = lower_expr(ctx, var_expr)?;
    ...
    let inv_tail = lower_expr(ctx, invariant)?;
    push_guard(ctx, inv_tail, inv_span, GuardKind::LoopInvariant);

    if let Some((v_before, v_span)) = before_variant {
        let v_after = lower_expr(ctx, variant.expect("variant expression lost"))?;
