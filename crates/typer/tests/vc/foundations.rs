...
#[test]
fn generates_mut_pre_vc_for_map_mut_alias_calls() {
    let src = r#"
        mut function put(m: Map<Int, Int>) -> Map<Int, Int>
    ...
    assert_eq!(vcs.len(), 1);
    let vc = &vcs[0];
    assert_eq!(vc.function, "put");
    assert!(vc.vc_id.starts_with("mut_pre:std::map::insert_mut:"));
