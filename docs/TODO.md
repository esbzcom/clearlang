...
- [x] 25.3 Unit testing and test runner with deterministic mock support [Gate D]
  - [x] 25.3.0 Publish Gate D design lock before implementation (scope, non-goals, deterministic contracts, fail-closed policy, completion criteria, and implementation order), and lock Gate D exit criterion: `clg test` is promoted from planned to shipped only after all required `25.3.x` gates are green. (`docs/design/phase-25.3.0-gate-d-design-lock.md`, `crates/cli/tests/phase25_gate_d_closeout.rs`)
  - [x] 25.3.0.1 Execute Gate D in dependency order: (1) contract/layout/schema foundations (`25.3.1`, `25.3.19`, `25.3.23`) -> (2) runner core (`25.3.2`, `25.3.10`, `25.3.11`, `25.3.14.1`) -> (3) machine-readable outputs (`25.3.3`, `25.3.9`, `25.3.21`) -> (4) deterministic mock system (`25.3.4`, `25.3.15`, `25.3.16`, `25.3.17`, `25.3.24`) -> (5) release/proved safety gates (`25.3.12`, `25.3.13`, `25.3.14`) -> (6) CI/release-precheck + platform determinism (`25.3.7`, `25.3.22`) -> (7) migration/quality/docs closeout (`25.3.6`, `25.3.8`, `25.3.18`, `25.3.25`, `25.3.26`). (`docs/design/phase-25.3.0-gate-d-design-lock.md`, `crates/cli/tests/phase25_gate_d_closeout.rs`)
  ...
  - [x] 25.3.5 Lock `clg test` proof-mode policy for unit workflows with one deterministic default mode first; keep advanced mode selection as documented follow-up. (`docs/design/phase-25.3.5-test-proof-mode-policy.md`, `docs/testing.md`, `crates/cli/tests/phase25_gate_d_closeout.rs`)
  - [x] 25.3.6 Define Gate D "enough test cases" acceptance matrix using risk/scenario completeness (parser/type/contracts/runtime/mock interactions; positive and negative cases) without fixed numeric thresholds. (`docs/testing-quality-matrix.md`, `docs/testing.md`, `crates/cli/tests/phase25_gate_d_closeout.rs`)
  - [x] 25.3.7 Integrate `clg test` as a required gate in CI and `xtask release-precheck` with deterministic report schema checks (no smoke-only fallback for Gate D completion). (`xtask/src/main/core.rs`, `xtask/src/main/artifacts_cli_models.rs`, `.github/workflows/ci.yml`, `crates/cli/tests/ci_workflow.rs`)
  - [x] 25.3.8 Defer deterministic migration/cutover from `clearlang-tests/` to canonical `tests/` until `clg test` + mock behavior is stable, and document temporary source-of-truth policy plus cutover trigger criteria. (`docs/testing.md`, `crates/cli/tests/phase25_gate_d_closeout.rs`)
  ...
  - [x] 25.3.18 Add balanced quality gate requiring non-mocked test coverage alongside mocked scenarios for critical paths (to avoid mock-only confidence). (`xtask/src/main/core.rs`, `examples/projects/testing/tests/test-plan.json`, `examples/projects/testing/tests/unit/discount_tests.clear`, `crates/cli/tests/phase25_gate_d_closeout.rs`)
  ...
  - [x] 25.3.22 Add cross-platform determinism gate for `clg test` outputs and report shape on milestone release targets (Windows baseline + Linux parity). (`crates/cli/tests/milestone3_test_parity.rs`, `.github/workflows/ci.yml`, `crates/cli/tests/ci_workflow.rs`, `crates/cli/tests/milestone3_release_gate.rs`)
  ...
  - [x] 25.3.25 Extend deterministic runtime safety policy for test workers beyond timeout (memory/fuel limits, crash handling, and deterministic status mapping). (`crates/cli/src/commands/test.rs`, `docs/testing.md`, `docs/diagnostics.md`)
  - [x] 25.3.26 Update primary UX/docs/help contracts after `clg test` lands (README, release-process, IDE profile, diagnostics table, CLI help/about string) and add drift gates to prevent stale command-surface docs. (`crates/cli/src/main.rs`, `README.md`, `docs/release-process.md`, `docs/ide/vscode-cli-profile.md`, `crates/cli/tests/phase25_gate_d_closeout.rs`)
