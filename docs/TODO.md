...
- [ ] Propagate refinement constraints through expressions, calls, and returns; specify when constraints are checked vs discharged to VCs and how alias unfolding/subtyping works (alias base resolution + predicate typing/purity checks are implemented; obligation propagation still pending; see `docs/typing.md`).
