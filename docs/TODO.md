...
- [x] 25.3 Unit testing and test runner with deterministic mock support [Gate D]
  ...
  - [x] 25.3.28 Close Gate D negative-evidence gaps in `clg test` contract tests: add deterministic integration coverage for `C134` (discovery/layout/signature), `C135` (plan/schema governance), `C137` (timeout), and `C138` (runtime guardrails/crash capture).
