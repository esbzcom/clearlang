...
- `clg.project.json` (release defaults; generated by `clg strict init`)
...
This command generates missing strict preflight inputs, validates strict schemas, and writes `clg.project.json` with release defaults placeholders (`REQUIRED_RFC3339_UTC`, `REQUIRED_KEY_ID`) that must be replaced (or overridden via CLI flags) before production release.
...
- Release precheck gate (`25.2.12`) is fail-closed for local+CI release workflows: `cargo run -p xtask -- release-precheck` (`fmt --check` + lint + tests + `clg test` schema gate) must pass before strict signed publish flow.
