...
- [x] 28.3 Release evidence and provenance parity [Distribution Gate D]
  - [x] 28.3.0 Extend release manifest / strict import-map / verify-bundle evidence to include selected shared std package identities, digests, signer ids, and ABI claims.
...
- [x] 28.6 Shared std evidence model unification [Distribution Gate G] `Completed: 2026-06-05`.
  - [x] 28.6.0 Extract one authoritative schema-v2 shared-std lock model + validator from the runtime loader into a reusable command-layer module rather than keeping release/import-map/runtime-specific raw decoders. (`crates/cli/src/commands/shared_std_lock.rs`, `crates/cli/src/commands/run/package_loader/{prelude.rs,lockfile.rs}`, `crates/cli/src/commands/mod.rs`) `Completed: 2026-06-05`.
  - [x] 28.6.1 Expand the authoritative validated model to retain every downstream field Phase 28 needs to project, not just runtime-link fields: package id, version, verified-ABI range, artifact path/digest/size, signer key id/algorithm/signed_at/signature, provenance digest/format, symbols, and shared-std dependencies. (`crates/cli/src/commands/shared_std_lock.rs`) `Completed: 2026-06-05`.
  - [x] 28.6.2 Refactor `clg release` to project release/provenance/verify-bundle shared-std evidence from the authoritative validated model and delete the current `RawReleaseSharedStd*` partial schema reader path. (`crates/cli/src/commands/release.rs`) `Completed: 2026-06-05`.
  - [x] 28.6.3 Refactor strict import-map generation to project shared-std evidence from the authoritative validated model and delete the current ad hoc JSON walker in `strict_import_map_shared_std_evidence`. (`crates/cli/src/commands/build/strict_artifact.rs`) `Completed: 2026-06-05`.
  - [x] 28.6.4 Make `pkg lock` writer/serializer/normalizer consume the same authoritative schema-v2 model used by runtime/release/import-map readers so shared-std ordering and field rules are enforced once at both write and read boundaries. (`crates/cli/src/commands/pkg/{prelude.rs,lockfile_loader.rs,artifacts.rs}`) `Completed: 2026-06-05`.
...
- [x] 29.4 `std::contract` shared-std promotion [Execution Gate B] `Completed: 2026-06-27`
  - [x] 29.4.0 Extend bundled shared-std package allowlists, canonical publication flow, and manifest/lock validation so `std::contract` is a supported shared package without changing the current default delivery mode. (`crates/cli/src/commands/modules/bundled_std_packages.rs`, `xtask/src/main/core/artifacts_and_vendor.rs`, `xtask/src/main/artifacts_cli_models.rs`, `crates/cli/src/commands/release_defaults/tests.rs`) `Completed: 2026-06-27`
  - [x] 29.4.1 Extend release, verify-bundle, runtime-loader, import-migration, and xtask publish coverage for the bounded `std::contract` surface under the same fail-closed trust, ABI, digest, provenance, and replay rules as existing supported packages. (`crates/cli/src/commands/modules/bundled_std_packages.rs`, `xtask/src/main/tests.rs`, `crates/cli/tests/cli_it/imports/std_and_edges.rs`) `Completed: 2026-06-27`
...
- [x] 29.5 Chain-adapter production ordering lock [Planning Gate D] `Completed: 2026-06-27`
...
- [x] 29.7 Remaining chain-adapter promotions [Execution Gate D] `Completed: 2026-06-27`
  - [x] 29.7.0 Promote the remaining approved chain-adapter packages one at a time using the same bounded contract from 29.5; no bulk multi-package activation is allowed without per-package evidence parity. `Completed: 2026-06-27`
    ...
    - [x] `std::cosmos`: promote the final approved chain-adapter package with the same per-package evidence parity before closing 29.7.0. (`crates/cli/src/commands/modules/bundled_std_packages.rs`, `xtask/src/main/core/artifacts_and_vendor.rs`, `xtask/src/main/artifacts_cli_models.rs`, `xtask/src/main/tests.rs`, `crates/cli/tests/cli_it/diagnostics/release_command_pipeline.rs`) `Completed: 2026-06-27`
  - [x] 29.7.1 Update docs, operational playbooks, and compatibility/deprecation notes after each chain-adapter promotion so the supported surface stays exact and audit-friendly. (`docs/release/shared-std-user-guide.md`, `docs/release/shared-std-operations.md`, `docs/release-process.md`, `docs/design/phase-28.9.1-shared-std-production-decision.md`, `docs/design/phase-28.9.2-shared-std-product-contract-lock.md`, `docs/evidence/phase-28.8-shared-std-support-readiness.md`) `Completed: 2026-06-27`
