...
- `15_str_literal.lumi`: defines a function returning `String` (parse/type only).
- `16_namespaced_call.lumi`: demonstrates namespaced callee syntax (parse-only).
- `17_hello_str.lumi`: hello world returning a `String` from `main` (parse/type only; codegen in Phase 5).
