...
fn print_help() {
    ...
    println!(
        "  shared-std-publish --package-id std::text|std::int|std::sequence|std::codec [--version X.Y.Z] --signed-at RFC3339Z [--out-dir DIR] [--registry-dir DIR] [--key-id ID] [--trusted-anchor-id ID] [--statement-format FORMAT] [--abi-major N] [--abi-minor-min N] [--abi-minor-max N]"
    );
