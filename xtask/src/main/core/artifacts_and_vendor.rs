        ...
        match raw_args[idx].as_str() {
            "--package-id" => {
                ...
                if !is_supported_shared_std_package_id(value.as_str()) {
                    return Err(format!(
                        "unsupported `--package-id` `{}`; expected one of: std::text, std::int, std::sequence, std::codec, std::contract, std::eth",
                        value
                    ));
                }
...
fn is_supported_shared_std_package_id(package_id: &str) -> bool {
    matches!(
        package_id,
        "std::text" | "std::int" | "std::sequence" | "std::codec" | "std::contract" | "std::eth"
    )
}
